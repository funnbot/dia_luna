...
public class SpriteFramesBuilder : IDisposable
{
    ...
    public SpriteFramesBuilder AddFrame(int x, int y, float duration = 1.0f) => AddFrame(new Vector2I(x, y), duration);

    private Rect2I CoordToFrameRect(Vector2I coord) =>
        new(coord * _frameSize, _frameSize, _frameSize);

    public SpriteFrames Build() => _sf;
